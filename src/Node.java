...
public class Node<T> {
	...
	boolean addNode(Node node) {
		...
		} else if (amIASuffix(node) || (tokenSequence.size() == 0)) // note that the empty sequence is always a suffix!
		{
			...
			children.add(node); // 1. try to add the node to all the children nodes. the pseudo recursive
		...
		} else if (!found && (tokenSequence.size() == tokenSequence.size() - 1)) // If NOT found and the length of
...
//		System.out.print(numSpacesBefore); //print the token
		}
...
//	if (tokenSequence.equals(getTokenSequence)) {
//	amI = true;
//	}
